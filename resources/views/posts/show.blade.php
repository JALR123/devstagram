...
@section('contenido')
    <div class="container mx-auto md:flex gap-5">
        <div class="md:w-1/2">
            ...
            <div>
                <a href="{{route('posts.index', $post->user)}}"><p class="font-bold">{{ $post->user->username }}</p></a>
        ...
        <div class="md:w-1/2">
            <div class="shadow bg-white p-5 mb-5">
                ...
                <form action="{{route('comentarios.store', ['user' => $user, 'post' => $post])}}" method="POST">
                    @csrf
                    <div class="mb-5">
                        <label for="comentario" class="mb-2 block uppercase text-gray-500 font-bold">Mensaje</label>
                        <textarea id="comentario" name="comentario"  class="border p-3 w-full rounded-lg @error('comentario')
                        border-red-500
                        @enderror"></textarea>
                        @error('comentario')
                ...
                <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                    @if ($post->comentarios->count())
                        @foreach ($post->comentarios as $comentario )
                                <div class="p-5 border-gray-300 border-b border-t">
                                    <div class="flex gap-2 items-center">
                                        <img class="rounded-full w-10" src="{{asset('perfiles/' . $comentario->user->imagen)}}"  alt="">
